...
- Portfolio node constellation keeps `Render Stats` and `API` as standalone launcher nodes (no nested `Systems` parent), preserving direct one-tap access to both terminal workflows.
...
- Every `API` click now appends a fresh manifest request block at the bottom of terminal history (timestamped), replaying line-by-line like a new backend call.
- The standalone `Render Stats` launcher node is terminal-native (no separate dashboard navigation from the portfolio node): it queries `/api/site-performance/render/overview` from the shared backend and streams service/summary output line-by-line into the same terminal stack.
- API and Render Stats terminal streams now prepend command-style execution lines (`run backend.manifest ...`, `run render.meta ...`) to reinforce terminal-call semantics.
...
- Terminal loading affordances are now explicit in both surfaces: portfolio prompt shows inline thinking state while Render Stats waits on API response, and TFT Coaching terminal shows inline thinking status during GPT generation/refresh waits.
...
- Portfolio launcher link assumptions:
  - production node links: `https://tftduos.brianz.dev` (TFT) and `https://api.brianz.dev` (shared backend + Render Stats terminal data source)
  - local host node links: `http://localhost:4173` (TFT) and `http://localhost:3001` (shared backend + Render Stats terminal data source)
